...
const TableRowCustom = ({
	...
	const handleClickAction = async (action: string) => {
		if (action === 'activate' || action === 'deactivate') {
			await updateSubcategory(data?._id, {
				active: action === 'activate' ? true : false,
			});
			await getSubcategories(searchValue, active, sort, paginationLink);
		} else if (action === 'delete') {
			await deleteSubcategory(data?._id);
			await getSubcategories(searchValue, active, sort, paginationLink);
		}
	};
	...
	return (
		<Fragment>
			<TableRow>
				...
				<TableCell>{data.parentCategory.name || 'None'}</TableCell>
						...
						<DropdownMenuContent>
							...
							>
								<DropdownMenuItem
									...
									onClick={() => setSelectedSubcategory(data)}
								>
									Edit
								</DropdownMenuItem>
							</DialogCustom>
